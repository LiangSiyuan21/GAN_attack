...
import pdb
from mmdet_v2200.core import bbox2result
...
class BlackBoxAttack(object):
    def __init__(self, max_loss_queries=np.inf,
                 ...
                 epsilon=0.5, p='inf', lb=0., ub=1., name = "nes", attack_model=None, attack_mode=None, attack_logistics=None, loss=None, targeted=None, ori_img=None, model_name=None, zeta=None, lambda1=None, patch_attack=None, keypoints_models=None, gaussian=None,
        ...
        if not (self.gaussian is None):
            self.ori_img_iter_0 = ori_img
        self.model_name = model_name
        self.zeta = zeta
        self.lambda1 = lambda1
        self.patch_attack = patch_attack
        self.keypoints_models = keypoints_models
        self.visual_iters_path = visual_iters_path
        self.records_flag = None
        ...
        # self.ensemble_iters = ensemble_iters
    ...
    def run(self, data, loss_fct, early_stop_crit_fct, vis_attack_step=None, log=None, results_records=None, quires_records=None, gt_info=None):
        ...
        if not (self.gaussian is None):
            img_ = np.array(self.ori_img_iter_0).copy()
            img_ = img_ / 255.0
            img_ = img_.transpose(0, 2, 3, 1)
            # noise = np.random.normal(MEAN, STD, img_.shape[1:])
            noise = np.random.normal(MEAN, STD, img_.shape)
            gaussian_out = img_ + noise*0.02
            gaussian_out = np.clip(gaussian_out, 0, 1)
            gaussian_out = np.float32(gaussian_out*255).transpose(0, 3, 1, 2)
            self.ori_img = torch.from_numpy(gaussian_out)
        ...
        elif self.name == 'IoUmgaGT' or self.name == 'IoUmgaGTFP' or self.name == 'IoUmgaGTFPiter' \
            or self.name == 'IoUmgaGTTPADDFPiter':
            results_records_iter_list = demo_utils.results_records_iter_list_for_square
        elif self.name == 'IoUss':
            results_records_iter_list = demo_utils.results_records_iter_list_for_square
        elif self.name == 'DCmgaGTTPADDFPiter':
            results_records_iter_list = demo_utils.results_records_iter_list_for_square
        elif self.name == 'IoUssGT' or self.name == 'IoUssGTFPiter' or self.name == 'IoUssGTTPADDFPiter':
            results_records_iter_list = demo_utils.results_records_iter_list_for_square
        ...
        if 'FP' in self.name:
            _, _, labels_all = demo_utils.get_bboxes_scores_and_labels(result, ncls=80)
            objects_all = np.unique(labels_all)

            objects_TP = objects_clean.copy()
        ...
        if isinstance(data['img'][0], torch.Tensor):
            ...
            xs = data['img'][0].data[0].numpy().transpose(0, 2, 3, 1)
        xs_t = t(xs)
        ...
        if quires_records is not None:
            temp = results_records_iter_list.index(0)
        ...
        elif 'GT' in self.name and 'FP' in self.name:
            clean_info = [bboxes_clean, bbox_scores_clean, labels_clean, objects_TP, objects_FP, labels_dic]
        elif 'GT' in self.name and gt_info is not None:
        ...
        while True:
            if self.name == "IoUnes" or self.name == "IoUzosignsgd" or self.name == "bandit":
                ...
                if np.any(num_loss_queries >= self.max_loss_queries):
                    if results_records is not None and quires_records is not None:
                        ...
                        if  results_records_iter_list[self.index_start] <= num_loss_queries:             
                            results_records[self.index_start].append(result[0])
                            quires_records[self.index_start].append(num_loss_queries)
            ...
            if np.any(num_crit_queries >= self.max_crit_queries):
                ...
                break
            if np.all(dones_mask): # True: all elements are True
                if log is not None:
                ...
                if results_records is not None and quires_records is not None:
                    ...
                    with torch.no_grad():
                        result = self.attack_model(return_loss=False, rescale=True, **data)
                    if self.index_start == stop_index:
                        results_records[self.index_start].append(result[0])
                        quires_records[self.index_start].append(num_loss_queries)
                    if self.index_start > stop_index:
                        break
                    else:          
                        for stop_idx in range(self.index_start, stop_index):
                            results_records[stop_idx].append(result[0])
                            quires_records[stop_idx].append(num_loss_queries)
                break


            ##updated x here
            self.results_records_iter_list = results_records_iter_list
            if self.name == 'IoUmgaGT' or self.name == 'IoUmgaGTFP' or self.name == 'IoUmgaGTFPiter'\
                or self.name == 'IoUmgaGTTPADDFPiter':
                start = time.time()
                sugg_xs_t, num_loss_queries_per_step = self._suggest(xs_t, loss_fct, img_metas, clean_info)
                print(time.time()-start)
                sugg_xs_t = sugg_xs_t.cpu().numpy().transpose(0,2,3,1)
                sugg_xs_t = torch.from_numpy(sugg_xs_t)
                xs_t = self.proj_replace(xs_t, sugg_xs_t, t(np.zeros(1).astype(np.float32)))
            ...
            if 'mga' in self.name:
                dones_mask = early_stop_crit_fct(self, xs_t[np.argmax(self.pop_fitness)][np.newaxis, :, :, :].cpu().numpy(), img_metas, clean_info)
                # its += 1
                self.is_new_batch = False

                if len(img_metas) != 1:
                    data['img'][0] = torch.FloatTensor(xs_t[np.argmax(self.pop_fitness)][np.newaxis, :, :, :].cpu().numpy().transpose(0,3,1,2))
                    data['img'][1] = torch.FloatTensor(xs_t[np.argmax(self.pop_fitness)][np.newaxis, :, :, :].cpu().numpy().transpose(0,3,1,2))
                else:
                    data['img'][0] = torch.FloatTensor(xs_t[np.argmax(self.pop_fitness)][np.newaxis, :, :, :].cpu().numpy().transpose(0,3,1,2))
            else:
            ...
            if results_records is not None and quires_records is not None:
                ...
                # print('records_num is {}'.format(records_num))
                # if len(results_records[self.index_start]) != len(results_records[self.index_start-1]):
                if (len(results_records[self.index_start])+1) == len(results_records[self.index_start-1]):
                    if records_num >= results_records_iter_list[self.index_start]:
                        with torch.no_grad():
                            result = self.attack_model(return_loss=False, rescale=True, **data)                
                        results_records[self.index_start].append(result[0])
                        quires_records[self.index_start].append(np.array(records_num))
                ...
                # elif (records_num-1 in results_records_iter_list) and (not self.records_flag):
                #     records_list_index = results_records_iter_list.index(records_num-1)
                #     with torch.no_grad():
                #         result = self.attack_model(return_loss=False, rescale=True, **data)                
                #     results_records[records_list_index].append(result[0])
                #     quires_records[records_list_index].append(np.array(records_num))
