...
class IoUSSAttack(BlackBoxAttack):
    def __init__(self, max_loss_queries, epsilon, p, p_init, lb, ub, name, attack_model, attack_mode, attack_logistics, loss, targeted, ori_img, model_name, zeta, lambda1, patch_attack, keypoints_models, square_expansion, attack_parallel, square_init, visual_iters_path, ensemble_models=None, gaussian=None):    
        super().__init__(
        ...
        if self.square_init is not None and self.square_init != 'False':
            self.square_init_method = self.square_init.split('_')[0]
            self.ensemble_iters = int(self.square_init.split('_')[2])
        ...
        if self.p == 'inf':
            if self.is_new_batch:
                ...
                if self.square_init is None or self.square_init == 'False':
                    ...
                    # init_delta = np.random.choice([-0.000001, 0.000001], size=[xs.shape[0], c, 1, w])
                    xs = xs + init_delta
                elif 'mdtifgsm' in self.square_init:
                    attack = TIFGSM(self.ensemble_models, eps=12.5/255, alpha=3/255, steps=self.ensemble_iters, decay=1.0, resize_rate=0.9, diversity_prob=0.7, random_start=True, ub=self.ub, lb=self.lb)
                    gt_bboxes = clean_info[0].astype(np.float32)*img_metas[0].data[0][0]['scale_factor']
                    gt_labels = clean_info[2].astype(np.int64)
                    adv = attack(xs, (gt_bboxes, gt_labels, img_metas))
                    init_delta = adv[1].detach().cpu().numpy() - xs
                    xs = xs + init_delta                
                else:
            ...
            for i_img in range(xs.shape[0]):
                ...
                for count, (center_h, center_w) in enumerate(zip(center_hs, center_ws)):
                    ...
                    # prevent trying out a delta if it doesn't change x_curr (e.g. an overlapping patch)
                    if not self.flip_flag:
                        # print(np.abs(np.clip(x_window + deltas[i_img, :, center_h:center_h+s, center_w:center_w+s], self.lb, self.ub) - x_best_window))
                        while np.sum(np.abs(np.clip(x_window + deltas[i_img, :, center_h:center_h+s, center_w:center_w+s], self.lb, self.ub) - x_best_window) < 10**-7) == c*s*s:
                            deltas[i_img, :, center_h:center_h+s, center_w:center_w+s] = np.random.choice([-self.epsilon, self.epsilon], size=[c, 1, 1])
                            self.flip_deltas = deltas
                        ...
                        if self.flip_zone == 'h1':
                            flip_deltas_sign = np.ones((1, c, s, s), dtype='float32')
                            flip_deltas_sign[:, :, flip_size:, :] = flip_deltas_sign[:, :, flip_size:, :] * -1.0
                            # print(deltas[i_img, :, center_h:center_h+s, center_w:center_w+s].shape)
                            # print(self.flip_deltas[i_img, :, center_h:center_h+s, center_w:center_w+s].shape)
                            temp1 = deltas[i_img, :, center_h:center_h+s, center_w:center_w+s]  
                            temp2 = self.flip_deltas[i_img, :, center_h:center_h+s, center_w:center_w+s]
