...
class FastModel(BaseModel):
    ...
    def backward_G(self, real_a_fea):
        ...
        for index, name in enumerate(self.loss_names):
            ...
            if name == 'G_Fea_min_Gaussi_L2':    
                ...
                random_noise_fea = torch.randn(real_a_fea[2].shape).to(self.device)
                random_noise_fea = random_noise_fea*eps
                noised_fea =random_noise_fea + real_a_fea[2]
            ...
            if name == 'G_cls_min':
                total_loss_dict = self.target_dict['model'](return_loss=True, **{'img_metas':self.target_dict['data']['img_metas'], 'img':self.fake_B}, gt_bboxes=self.target_dict["gt_bboxes"].unsqueeze(0), gt_labels=self.target_dict["gt_labels"].unsqueeze(0))
                self.loss_G_cls_min = torch.exp(total_loss_dict['loss_cls'] * (-1.0)) * float(self.loss_paras[index])

                self.loss_G = self.loss_G + self.loss_G_cls_min.cuda()
            
                self.loss_G_bbox_min = torch.exp(total_loss_dict['loss_bbox'] * (-1.0)) * float(self.loss_paras[index])
                self.loss_G = self.loss_G + self.loss_G_bbox_min.cuda()

                self.loss_G_obj_min = torch.exp(total_loss_dict['loss_obj'] * (-1.0)) * float(self.loss_paras[index])
    ...
    def optimize_parameters(self):
        ...
        # print (real_a_fea[0].min().item())
        # print("--------1-------")
        # print (real_a_fea[1].max().item())
        # print (real_a_fea[1].min().item())
        # print("--------2-------")
        # print (real_a_fea[2].max().item())
        # print (real_a_fea[2].min().item())
        while self.loss_G_cls_min > 100.0 and i < 10:
            self.forward()
            self.optimizer_G.zero_grad()        # set G's gradients to zero
            self.backward_G(real_a_fea)                   # calculate graidents for G
