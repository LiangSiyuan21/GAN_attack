...
class TranslateModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['real_A_np', 'fake_B_np']
        ...
        self.loss_G_bbox_min = 1000.0
        self.loss_G_obj_min = 1000.0
        # define networks (both generator and discriminator)
        # self.netG = 
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, "resnet_9blocks", opt.norm,
    ...
    def forward(self):
        ...
        self.fake_B = perturbation + self.real_A
        # self.fake_B = torch_innormalize(self.fake_B) * 255.0
        # self.fake_B = Variable(torch_ori_size(perturbation) + self.real_A, requires_grad=True)
        self.fake_B = torch.clip(self.fake_B, min=0.0, max=255.0) # G(A) + A
    ...
    def backward_G(self):
        ...
        for index, name in enumerate(self.loss_names):
            ...
            if name == 'P_L2':
                self.loss_P_L2 = self.criterionL2(self.real_A, self.fake_B) * float(self.loss_paras[index])
                self.loss_G = self.loss_G + self.loss_P_L2
            # if name == 'G_L1_fb_rB':
            # # Second, G(A) + A = B
            ...
            if name == 'G_Fea_min_Gaussi_L2':    
                ...
                random_noise_fea = torch.randn(self.real_a_fea[2].shape).to(self.device)
                random_noise_fea = random_noise_fea*eps
                noised_fea =random_noise_fea + self.real_a_fea[2]
                # self.loss_G_Fea_min_Gaussi_L2 = self.criterionL2(fake_b_fea[2], self.target_dict['tar_features'][4]) * float(self.loss_paras[index])
                self.loss_G_Fea_min_Gaussi_L2 = self.criterionL2(fake_b_fea[2], noised_fea) * float(self.loss_paras[index])
                print (self.loss_G_Fea_min_Gaussi_L2.item())
                self.loss_G = self.loss_G + self.loss_G_Fea_min_Gaussi_L2.cuda()
            
            if name == 'G_cls_min':
                # Fourth, f(G(A)+A) /= f(A)
                # with torch.no_grad():
                self.loss_G_cls_min = torch.exp(self.target_dict['model'](return_loss=True, **{'img_metas':self.target_dict['data']['img_metas'], 'img':self.fake_B}, gt_bboxes=self.target_dict["gt_bboxes"].unsqueeze(0), gt_labels=self.target_dict["gt_labels"].unsqueeze(0))['loss_cls'] * (-1.0)) * float(self.loss_paras[index])
                # self.loss_G_cls_min = self.target_dict['model'](return_loss=True, **{'img_metas':self.target_dict['data']['img_metas'], 'img':self.fake_B}, gt_bboxes=self.target_dict["gt_bboxes"].unsqueeze(0), gt_labels=self.target_dict["gt_labels"].unsqueeze(0))['loss_cls'] * (-1.0) * float(self.loss_paras[index])
                self.loss_G = self.loss_G + self.loss_G_cls_min.cuda()
            
            if name == 'G_bbox_min':
                # Fourth, f(G(A)+A) /= f(A)
                # with torch.no_grad():
                self.loss_G_bbox_min = torch.exp(self.target_dict['model'](return_loss=True, **{'img_metas':self.target_dict['data']['img_metas'], 'img':self.fake_B}, gt_bboxes=self.target_dict["gt_bboxes"].unsqueeze(0), gt_labels=self.target_dict["gt_labels"].unsqueeze(0))['loss_bbox'] * (-1.0)) * float(self.loss_paras[index])
                self.loss_G = self.loss_G + self.loss_G_bbox_min.cuda()
            
            if name == 'G_obj_min':
                # Fourth, f(G(A)+A) /= f(A)
                # with torch.no_grad():
                self.loss_G_obj_min = torch.exp(self.target_dict['model'](return_loss=True, **{'img_metas':self.target_dict['data']['img_metas'], 'img':self.fake_B}, gt_bboxes=self.target_dict["gt_bboxes"].unsqueeze(0), gt_labels=self.target_dict["gt_labels"].unsqueeze(0))['loss_obj'] * (-1.0)) * float(self.loss_paras[index])
                self.loss_G = self.loss_G + self.loss_G_obj_min.cuda()
    ...
    def optimize_parameters(self):
        ...
        i = 0
        self.loss_G_cls_min = 1000.0
        self.real_a_fea = self.target_dict['model'](return_loss=False, rescale=False, attack_mode=True, get_features=True, **{'img_metas':self.target_dict['data']['img_metas'], 'img':[self.real_A]})      
        while self.loss_G_cls_min > 100.0 and i < 10:
            self.forward()
