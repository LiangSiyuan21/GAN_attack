...
**FP16 related methods are imported from mmcv instead of mmdet_v2200. (#3766, #3822)**
Mixed precision training utils in `mmdet_v2200.core.fp16` are moved to `mmcv.runner`, including `force_fp32`, `auto_fp16`, `wrap_fp16_model`, and `Fp16OptimizerHook`. A deprecation warning will be raised if users attempt to import those methods from `mmdet_v2200.core.fp16`, and will be finally removed in V2.8.0.
...
**Highlights**
- The CUDA/C++ operators have been moved to `mmcv.ops`. For backward compatibility `mmdet_v2200.ops` is kept as warppers of `mmcv.ops`.
...
- Move CUDA/C++ operators into `mmcv.ops` and keep `mmdet_v2200.ops` as warppers for backward compatibility (#3232)(#3457)
...
- Use optimizer constructor in mmcv and clean the original implementation in `mmdet_v2200.core.optimizer` (#2947)
