...
class DispersionAttack_gpu(object):
    ...
    def __init__(self, model, epsilon=16/255., step_size=0.004, steps=10, lb=0, ub=1):
    ...
    def __call__(self, data, attack_layer_idx=-1, internal=[]):
        for p in self.model.parameters():
            p.requires_grad = False
        self.model.eval()
        X_nat_var = data['img'][0]
        X_var = torch.clone(X_nat_var)
        for i in range(self.steps):
            X_var.requires_grad = True
            data['img'][0] = X_var
            X_var.retain_grad()
            internal_features = self.model(return_loss=False, rescale=True, get_features=True, **data)
            # internal_features, pred = self.model(X_var, internal=internal)
            ...
            X_var = torch.clamp(X_var, self.lb, self.ub)
