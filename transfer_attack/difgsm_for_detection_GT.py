...
from transfer_attack.attacks.difgsm import DI2FGSM
...
def main():
    ...
    #     continue
        ...
        # define the attacker
        # DI2FGSM
        attack = DI2FGSM(model, eps=12.5/255, alpha=3/255, steps=int(args.n_iter), decay=0.0, resize_rate=0.9, diversity_prob=0.5, random_start=False, ub=ub, lb=lb)

        # M-DI2FGSM
        # attack = DI2FGSM(model, eps=12.5/255, alpha=3/255, steps=int(args.n_iter), decay=1.0, resize_rate=0.9, diversity_prob=0.5, random_start=False, ub=ub, lb=lb)

        adv = attack(data, (gt_bboxes_tor, gt_labels_tor))
