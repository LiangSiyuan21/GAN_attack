...
from mmdet_v2200.core import PolygonMasks
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                ...
                img, w_scale, h_scale = mmcv.imresize(
                    ...
                    return_scale=True,
                    backend=self.backend)
            if results['have_ad_img']:
                if self.keep_ratio:
                    adv_img, scale_factor = mmcv.imrescale(
                        results['adv_img'],
                        results['scale'],
                        return_scale=True,
                        backend=self.backend)
                    # the w_scale and h_scale has minor difference
                    # a real fix should be done in the mmcv.imrescale in the future
                    new_h, new_w = adv_img.shape[:2]
                    h, w = results['adv_img'].shape[:2]
                    w_scale = new_w / w
                    h_scale = new_h / h
                else:
                    adv_img, w_scale, h_scale = mmcv.imresize(
                        results['adv_img'],
                        results['scale'],
                        return_scale=True,
                        backend=self.backend)
                results['adv_img'] = adv_img
            results[key] = img
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            for key in results.get('img_fields', ['img']):
                ...
                if results['have_ad_img']:
                    results['adv_img'] = mmcv.imflip(
                    results['adv_img'], direction=results['flip_direction'])
            # flip bboxes
            for key in results.get('bbox_fields', []):
...
class Pad(object):
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if results['have_ad_img']:
                if self.size is not None:
                    padded_adv_img = mmcv.impad(
                        results['adv_img'], shape=self.size, pad_val=self.pad_val)
                elif self.size_divisor is not None:
                    padded_adv_img = mmcv.impad_to_multiple(
                        results['adv_img'], self.size_divisor, pad_val=self.pad_val)
                results['adv_img'] = padded_adv_img                
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            results[key] = mmcv.imnormalize(results[key], self.mean, self.std,
                                            self.to_rgb)
            if results['have_ad_img']:
                results['adv_img'] = mmcv.imnormalize(results['adv_img'], self.mean, self.std, self.to_rgb)                
