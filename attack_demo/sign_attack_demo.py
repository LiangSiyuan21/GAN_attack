# -*- encoding: utf-8 -*-
'''
@File    :   sign_attack_demo.py
@Time    :   2020/12/01 10:30:01
@Author  :   liangsiyuan 
'''
...
from blackbox_attack.utils.criterion import loss_fct, early_stop_crit_fct
...
from blackbox_attack.sign_attack import SignAttack
...
def parse_args():
    ...
    # attack argument
    parser.add_argument('--attack', type=str, default='sign_linf', choices=['sign_linf', 'sign_l2'], help='Attack.')
    ...
    parser.add_argument('--fd_eta', type=float, default=0.05)
...
def main():
    ...
    for index, data in enumerate(data_loader):
        ...
        if vis_attack_step is not None and vis_attack_step:
            queries, vis_result = attacker.run(data, loss_fct, early_stop_crit_fct, vis_attack_step=vis_attack_step, log=log)
        else:
    ...
    if rank == 0:
        ...
        if args.eval:
            ...
            log.logger.info(dataset.evaluate(results, **eval_kwargs))
        
    mean_quries = np.array(total_quires).sum(0) / len(data_loader)
    mean_times =  np.array(total_times).sum(0) / len(data_loader)
    log.logger.info('{} images: mean quries {}, mean times {}'.format(len(data_loader), mean_quries, mean_times))
