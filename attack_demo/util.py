...
# results_records_iter_list_for_square = [0, 12, 22, 32, 42, 52, 62, 72, 82, 92]
...
def visual_iter_process(attacker, xs, ys, save_path):
    fig = plt.figure(figsize=(7,5))
    ax1 = fig.add_subplot(1, 1, 1)

    if attacker.name == 'IoUmgaGT' or attacker.name == 'IoUmgaGTFP' or attacker.name == 'IoUmgaGTFPiter':
        title = 'pop_size:' + str(attacker.pop_size) + ' cr:' + str(attacker.cross_rate) + ' mr:' + str(attacker.mutation_rate) + ' e_iters:' + str(attacker.iters)
    else:
        title = 'iter loss for ' + attacker.name + ' methods in process'

    p1 = pl.plot(xs, ys[0], 'r-', label = 'current loss')
    pl.legend()
    pl.xlabel('iters')
    pl.ylabel('loss')
    plt.title(title)

    p2 = pl.plot(xs, ys[1], 'b-', label = 'best loss')
    pl.legend()
    # plt.title('iter loss for ' + attacker.name + ' methods in process')

    plt.savefig(save_path)
    print('The iters process has been saved in ' + save_path)
    plt.close('all')
...
def get_objects_FP(TP_labels, All_labels):
    All_labels = list(All_labels)
    TP_labels = list(TP_labels)
...
def bbox_to_attack_points(h, w, clean_info, img_metas, s=None, get_proposal=False):
    ...
    x1_mask_0 = bboxes_scaled_clean[:, 0] > 0
    y1_mask = bboxes_scaled_clean[:, 1] < (h-s)
    y1_mask_0 = bboxes_scaled_clean[:, 1] > 0
    mask = x1_mask & y1_mask & y1_mask_0 & x1_mask_0
