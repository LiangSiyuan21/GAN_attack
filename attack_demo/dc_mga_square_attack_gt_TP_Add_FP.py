...
from blackbox_attack.utils.criterion import loss_fct, early_stop_crit_fct, early_stop_crit_fct_with_iou, loss_fct_with_TP_add_FP
...
from blackbox_attack.mga_ss_attack import MGASSAttack 
...
def parse_args():
    ...
    parser.add_argument('--divide_conquer_num', type=str, default=None)
    parser.add_argument('--divide_conquer_restart', type=str, default=None)
    parser.add_argument('--fitness_style', type=str, default=None)
    parser.add_argument('--pop_size', type=str, default=None)
    parser.add_argument('--cr', type=str, default=None)
    parser.add_argument('--ensemble_iters', type=str, default=None)
    parser.add_argument('--mr', type=str, default=None)
    parser.add_argument('--ensemble_models', type=str, default=None)
    parser.add_argument('--mga_iters', type=int, default=None)
    parser.add_argument('--loss_log', type=str, default=None)
    parser.add_argument('--mix_init', type=str, default=None)
    parser.add_argument('--gaussian', default=None)
...
def main():
    ...
    for m in args.ensemble_models.split(','):
        if m == 'FR':
            ensemble_config_path = 'configs_v2200/faster_rcnn/faster_rcnn_r50_fpn_2x_coco.py' 
            ensemble_checkpoint = args.checkpoint.split('checkpoints')[0] + 'checkpoints/faster_rcnn_r50_fpn_2x_coco_bbox_mAP-0.384_20200504_210434-a5d8aa15.pth'
            ensemble_cfg = Config.fromfile(ensemble_config_path)
            ensemble_cfg.model.pretrained = None
            ens_model = build_detector(ensemble_cfg.model, test_cfg=ensemble_cfg.get('test_cfg'))
            ebsemble_checkpoint = load_checkpoint(ens_model, ensemble_checkpoint, map_location='cpu')
            if 'CLASSES' in ebsemble_checkpoint['meta']:
                ens_model.CLASSES = ebsemble_checkpoint['meta']['CLASSES']
            else:
                ens_model.CLASSES = dataset.CLASSES
            ens_model = MMDataParallel(ens_model, device_ids=[0])
            #    ens_model.eval()
            ens_models.append(ens_model)
        elif m == 'SSD':
            ensemble_config_path = 'configs_v2200/ssd/ssd512_coco.py' 
            ensemble_checkpoint = args.checkpoint.split('checkpoints')[0] + 'checkpoints/ssd512_coco_20210803_022849-0a47a1ca.pth'            
            ensemble_cfg = Config.fromfile(ensemble_config_path)
            ensemble_cfg.model.pretrained = None
            ens_model = build_detector(ensemble_cfg.model, test_cfg=ensemble_cfg.get('test_cfg'))
            ebsemble_checkpoint = load_checkpoint(ens_model, ensemble_checkpoint, map_location='cpu')
    ...
    elif args.attack.split('_')[0] == 'IoUmgaGT' or args.attack.split('_')[0] == 'IoUmgaGTFP' or args.attack.split('_')[0] == 'IoUmgaGTFPiter' or args.attack.split('_')[0] == 'IoUmgaGTTPADDFPiter':
        results_records_iter_list = demo_utils.results_records_iter_list_for_square
    elif args.attack.split('_')[0] == "DCmgaGTTPADDFPiter":
        results_records_iter_list = demo_utils.results_records_iter_list_for_square
    ...
    #     continue
        ...
        attacker = eval('MGASSAttack')(
            ...
            divide_conquer_num=args.divide_conquer_num,
            divide_conquer_restart=args.divide_conquer_restart,
            fitness_style=args.fitness_style,
            mga_iters=args.mga_iters,
        ...
        if vis_attack_step is not None and vis_attack_step:
            queries, vis_result, results_records, queries_records = attacker.run(data, loss_fct_with_TP_add_FP, early_stop_crit_fct_with_iou, vis_attack_step=vis_attack_step, results_records=results_records, quires_records=quires_records, gt_info=[gt_bboxes, gt_scores, gt_labels, labels_dic])
        else:
